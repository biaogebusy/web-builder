...
export class BuilderEmptyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.content = {
      ...
      type: 'text',
      body: '<p>开启 <strong class="text-primary">Builder</strong> 后，可以从左侧选择组件拖动到想要的位置，甚至你可以在浏览前台任何页面时或者浏览 Storybook 页面时添加组件到预览页面，类似组件市场。</p><ul class="list-done"><li>可以快速复制整个页面的 JSON；</li><li>可以复制具体组件的 JSON；</li><li>可以直接编辑组件的 JSON，所见即所得；</li></ul>',
      spacer: 'lg',
