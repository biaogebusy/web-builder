...
<Meta title="开发/组件主题" id="create-theme" />
...
使用 mat.get-color-from-palette(palette,hue: default, $opacity: null) :从调色盘获取特定的颜色，可以指定色调和透明度。
...
@mixin hero-color($theme) {
  ...
  .hero-1-v2 {
    ...
    outline-color: mat.get-color-from-palette($accent, lighter);

    // 添加 `-contrast` 获取指定色盘的对比度颜色
    border-color: mat.get-color-from-palette($primary, '100-contrast');
  }
}
```
