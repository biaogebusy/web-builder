...
在`src/stories/builder/data`目录中维护了需要导入到 web builder 的数据:
...
- layout builder 弹窗基组件：`src/stories/builder/data/widgets-for-story.ts`
  ...
  新增组件或者每次你更新数据时，在storybook环境下，浏览器开发者面板会打印出对应的数据，复制到`src/app/modules/builder/data`对应的文件即可更新。
