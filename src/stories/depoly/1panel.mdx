...
<Meta title="部署/创建node应用" id="create-node" />
...
使用命令`npm run build` 打包应用，打包成功后会生成`dist`目录，把`dist`目录和根目录的`package.json` 上传到 1panel。

## 在 1panel 中新建 node 运行环境

<img src="/assets/storybook/assets/1panel-node.png" alt="1panel-node" width="85%" />
...
<img src="/assets/storybook/assets/1panel-proxy.png" alt="1panel-node" width="85%" />
...
1panel默认没有对`application/json`进行gzip压缩，在`OpenResty`的全局配置中，新增：
...
1. 因为媒体库静态图片资源会加载所有的图片，1panel默认请求频率要求会封掉IP，可以调高频率或者禁用WAF功能；
1. 关闭参数规则，影响后台数据保存；
1. 关闭其他中 SQL 和 XXS，影响富文本图片上传及 builder 的更新修改；
