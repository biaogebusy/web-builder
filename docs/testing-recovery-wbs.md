...
- **2026-07-25**:T8 批次 1 完成(25 个行为用例):layout-setting(模型合并/元素增删/确认后批量应用动画)、widget-picker(未选位置提示、widget/element 两种落位、粘贴复制组件并清剪贴板)、builder-version(storage 加载与跟随、切换版本+最新性检查、改名持久化)、manage-media(缺 uuid 提示、确认删除+刷新、取消不删、顺序批删+进度)、upload-media(拖拽上传回填、目录拒绝、复制地址)、comment-form(新增/回复/更新的 payload 组装与 token、失败提示)、login(OAuth redirect、手机号校验、验证码倒计时、失败消息)。
  - 踩坑记录:① 组件 standalone `imports` 里的 NgModule(如 MatDialogModule)提供的服务在组件注入器层,TestBed 级 provider 覆盖不到 → 用 `fixture.debugElement.injector.get(X)` + `vi.spyOn` 处理(layout-setting);② 行为断言前需 `fixture.detectChanges()` 同步跑 ngOnInit(zoneless 自动 CD 是异步调度);③ 断言副作用要看完整链路(manage-media 删除成功后 `onSearch` 会把 loading 重新置 true)。
- **2026-07-25**:T8 批次 2 完成(18 个行为用例):builder-toolbar 在存量 4 个保存路径用例上补齐缺口(空页面拦截、更新成功后导航+广播、全屏开关持久化、改标题保存/空标题不动);taxonomy(未登录拦截、新增+刷新+重置、新增失败提示、updateAttributes 更新、删除+刷新、重置清选中);user-setting(更新剥离确认密码、失败提示、登出关闭全部弹窗、头像空文件/超 5M 拦截、上传成功)。规避 formly 模板渲染风险:taxonomy 手动调 `ngOnInit()`、user-setting 不触发渲染,只测方法行为。
- **2026-07-25**:T8 批次 3 完成(17 个行为用例):page-list(初始化消费 pending 页、无 nid 拦截、加载/复制为模板、JSON 编辑守卫、页面设置弹窗参数、创建语言版本三分支——已存在译文/新译文草稿/取消不动);comment-item(回复/更新模式状态机、取消复位、编辑中其他评论保持可见);dynamic-form(无效表单只标 touched、提交携带 webform_id+表单值并在成功后重置、后端错误消息透传、无消息时兜底文案)。工厂 mock 返回类型放宽(`fetch`/`getNodes`/`getNodeByLink` → `Observable<any>`,`consumePageLoad` → `unknown`),便于行为测试 `mockReturnValueOnce` 自定义响应。
  - 发现的潜在 bug(未修,待确认):`comment-item.component.ts` `onDelete()` 在方法体内调用无参 `takeUntilDestroyed()`,运行时脱离注入上下文会抛 NG0203,删除评论功能疑似已损坏;spec 暂不覆盖该方法。
- **2026-07-25**:T8 批次 4 完成(19 个行为用例)+ **修复批次 3 发现的 bug**:`comment-item.onDelete()` 的 `takeUntilDestroyed()` 补上 `this.destroyRef` 参数(NG0203 消除,删除评论恢复可用),并补齐删除路径三个用例(按 field_name 端点删除+确认提示、失败提示用户状态、无 params 不发请求);comment-actions(更新/回复/删除三个输出事件、引用推入 commentQuote$、isMy 三分支判定);jsoneditor(页面 JSON 整页替换、组件 JSON 按 path 写回、无值不动、DOM 事件忽略/真实编辑置 loading、isSetting 持久化成功后提示+关抽屉、失败静默);code-editor(HTML 写回剥离 relationships、无 path 不动、自定义 JS 仅管理员且空值删键、fake timers 验证 2s 防抖保存流)。工厂 UtilitiesService mock 补 `loadScriptWithoutAmd`。
- **2026-07-26**:T8 批次 5 完成(13 个行为用例)+ **修复全仓 6 处"signal 当值用"bug**:`inject(USER)` 返回 Signal,`if (!this.user)` 恒假导致登录守卫全部失效——修复 `media-upload.service`(Quill 上传守卫)、`builder-toolbar.onSubmit`、`page-setting.onUpdate/deletePage`、`upload-media.dropped`、`link`(`:id` 替换分支);统一改为 `this.user()`。守卫用例同步补齐:media spec(未登录拦截 Quill 上传)、upload-media(未登录提示不上传)、builder-toolbar(未登录弹登录框不保存)、page-setting(更新/删除均先弹登录框)。新增行为覆盖:switch-preview(空页拒绝预览、切断点+关抽屉+广播、恢复默认图标);builder-showcase(复制组件写剪贴板 key、无 uuid 不弹确认、确认删除+折叠 showcase、取消保留、失败提示不折叠、插入组件后滚动定位并关闭)。工厂 UserService mock 补 `openLoginDialog`。comment-list 为纯展示组件,无行为可测,维持脚手架。
- **2026-07-26**:T8 批次 6 完成(35 个用例,5 个新服务级 spec):`user.service.check-show.spec`(角色门控六分支:无内容/无限制/未登录/持有角色/uid=1 超管/无角色);`node.service.access.spec`(checkReqRule 匿名拒绝、管理员放行、角色交集;checkNodeAccess 受限允许/拒绝、无规则公开);`api.service.spec`(pageUrl 白名单查询参数保留/丢弃、语言前缀解析与默认语回退、getUrlPath 拆分、Drupal views 空/满分页归一);`form.service.spec`(toFormGroup 普通/必填/多选拆分/日期区间、getwebFormData 合并、handleRangeDate 展平格式化);`route.service.spec`(isAbsolute 含协议相对、抽屉图片/iframe 两种部件、相对路径走路由、绝对与 manage/print/export 走 window.open、updateQueryParams 重写 URL、锚点 rel=drawer 加载进抽屉、普通锚点走路由)。核心服务的关键纯逻辑至此均有直接覆盖。
